...
fun testDay7Part1() = println(testInputDay7.parseTower().bottom)

fun String.parseTower(): Tower =
    split('\n')
        .filter(String::isNotEmpty)
        .map { "\\w+|\\d+".toRegex().findAll(it) }
        .map { it.toList().map(MatchResult::value) }
        .map { Program(it[0], it[1].toInt(), it.drop(2)) }
        .apply {
          forEach { program -> program.subPrograms = filter { it.name in program.subProgramNames } }
        }

typealias Tower = Collection<Program>

data class Program(
    val name: String,
    val weight: Int,
    val subProgramNames: Collection<String>,
    var subPrograms: Tower = emptySet()
)

val Tower.bottom get() = single { it.name !in flatMap(Program::subProgramNames) }

val inputDay7 = FileProvider.url("/inputDay7.txt")!!.readText()

fun day7Part1() = println(inputDay7.parseTower().bottom)
