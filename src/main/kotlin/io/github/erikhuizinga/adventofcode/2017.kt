...
fun main(args: Array<String>) = day9Part1()
...
fun testDay9Part1() = testInputsDay9.forEach { println("$it = ${scoreGarbageGroups(it)}") }

fun day9Part1() = println(scoreGarbageGroups(inputDay9))

fun scoreGarbageGroups(input: String): Int {
  var level = 0
  var score = 0
  var inGarbage = false
  ...
  input.forEachIndexed { index, char ->
    if (inGarbage) {
      if (char == '>' && input.take(index).takeLastWhile { it == '!' }.count() % 2 == 0) {
        inGarbage = false
      }
    } else {
      when (char) {
